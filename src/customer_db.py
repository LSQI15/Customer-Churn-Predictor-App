...
class Customer(Base):
    ...
    MultipleLines = Column(String(100), unique=False,
                           nullable=False)  # categorical with 3 classes ('No phone service', 'No', 'Yes')
    InternetService = Column(String(100), unique=False,
                             nullable=False)  # categorical with 3 classes ('DSL', 'Fiber optic', 'No')
    ...
    Contract = Column(String(100), unique=False,
                      nullable=False)  # categorical with 3 classes ('Month-to-month', 'One year', 'Two year')
    PaperlessBilling = Column(String(100), unique=False, nullable=False)  # binary (Yes/No)
    PaymentMethod = Column(String(100), unique=False,
                           nullable=False)  # categorical with 4 classes ('Electronic check', 'Mailed check','Bank transfer (automatic)','Credit card (automatic)'
...
def initial_ingest(args):
    """
    main function to conduct initial ingestion to the database; will ingest the database with args.num_records
    observations;
    :param args: Argparse args - should include args.engine_string, args.config, args.num_records
    :return: None
    """
    try:
        ...
        # add existing records from the preprocessed data to the database
        df = csv_reader(**config['run_create_db']['init_df']).drop('Churn', axis=1).head(args.num_records)
